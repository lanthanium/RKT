...
class Tweet {
    ...
    get source() {
        ...
        if (this.text.toUpperCase().startsWith('JUST COMPLETED') ||
            this.text.toUpperCase().startsWith('JUST POSTED'))
            return 'completed_event';
        if (this.text.toUpperCase().includes('ACHIEVE') ||
            this.text.toUpperCase().includes('MET MY'))
            return 'achievement';
        if (this.text.toUpperCase().includes('WATCH MY') ||
            this.text.toUpperCase().includes('RIGHT NOW'))
            return 'live_event';
    ...
    get written() {
        ...
        if (this.text.toUpperCase().includes('CHECK IT OUT!'))
            return false;
        if (this.text.toUpperCase().startsWith('ACHIEVE'))
            return false;
        if (this.text.toUpperCase().includes('TOMTOM'))
            return false;
        if (this.text.toUpperCase().startsWith('WATCH MY'))
            return false;
        return true;
    }
    get writtenText() {
    ...
    get activityType() {
        ...
        if (text.includes('workout'))
            text = text.substring(0, text.indexOf('workout') - 1);
    ...
    get weekDayEnd() {
        if (this.createdAt.includes('Sat') || this.createdAt.includes('Sun'))
            return "Weekend";
        else
            return "Weekday";
    }
